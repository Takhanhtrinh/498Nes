...
static inline u16 getNameTableAddress() { 
  ...
  if (val == 0) return 0x2000;
  else if (val == 1) return 0x2400;
  else if (val == 2) return 0x2800;
  else if (val == 3) return 0x2C00;
  else 
    assert(0 && "getNameTableAddress address invalid");
  return 0;
}
static inline u8 getAddressIncrement() { return ((ppu.ppu_ctrl & PPU_ADDRESS_INCREMENT_MASK) >> 2) == 0 ? 1 : 32; }
static inline u16 getSpritePatternTableAddress() { return ((ppu.ppu_ctrl & SPRITE_PATTERN_ADDRESS_MASK) >> 3) == 0 ? 0x0000 : 0x1000; }
static inline u16 getBackgroundPatternTableAddress() { return ((ppu.ppu_ctrl & BACKGROUND_PATTERN_ADDRESS_MASK) >> 4) == 0 ?  0x0000 : 0x1000 ; }
static inline u8 getSpriteSize() { return ((ppu.ppu_ctrl & SPRITE_PATTERN_ADDRESS_MASK) >> 5) == 0 ? SPRITE_8X8 : SPRITE_16X8; }
...
static inline u8 getBackgroundLeftMost() { return (ppu.ppu_mask & BACKGROUND_8PX_LEFTMOST) >> 1; }
static inline u8 getSpriteLeftMost() { return (ppu.ppu_mask & SPRITE_8PX_LEFTMOST) >> 2;}
static inline u8 getShowBackground() { return (ppu.ppu_mask & SHOW_BACKGROUND) >> 3; }
...
//OAMADDR
static inline void writeOAMdata(u8 val) { ppu.oam_buffer[ppu.oam_addr++] = val; }
//OAMDATA
...
u8 getScanlineState();
static inline u16 fetchNameTable() { return 0x2000 | (ppu.ppu_address & 0x0FFF); }
static inline u16 fetchAttribute() {return 0x23C0 | (ppu.ppu_address & 0x0C00) | ((ppu.ppu_address >> 4) & 0x38) | ((ppu.ppu_address >> 2) & 0x07);}

static inline u16 getBGcolorAddress(u8 val) {return  0x3F00  + (val * 4);}
static inline u16 getSpriteColorAddress(u8 patternIndex, u8 value) {return 0x3F10 + (patternIndex * 4) + value;}
void fetchLowByte();
void fetchHighByte();
void printNameTable();
void generatePixel();
